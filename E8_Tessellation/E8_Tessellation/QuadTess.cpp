...
QuadTess::QuadTess(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
	initBuffers(device);
}
...
void QuadTess::sendData(ID3D11DeviceContext* deviceContext, D3D_PRIMITIVE_TOPOLOGY top)
{
	unsigned int stride;
	unsigned int offset;
...
void QuadTess::initBuffers(ID3D11Device* device)
{
	VertexType* vertices;
